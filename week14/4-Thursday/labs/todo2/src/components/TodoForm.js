...
const TodoForm = ({todo, editMode, pageType, }) => {
  ...
  const [task, setTask] = useState(()=> editMode ? todo.task : ''); //turnary statement //means edit mode is true so we display task, other wise we display empty string
  ...
  const handleSubmit = (e) => {
    ...
    let todoID = editMode ? todo.id : uuidv1(); //edit mode is it true?? 
